...
\env{frame}
{
	\env{center}
	{
		\env{tikzpicture}
		{
			...
			\only<1-2> { \path[draw, thick] (1) -- (2); }
			\only<3-11> { \path[draw, thick, blue] (1) -- (2); }
			\node[fill = white] at (2,1) {$1$};

			\only<1-3> { \path[draw, thick] (2) -- (4); }
			\only<4-> { \path[draw, thick, blue] (2) -- (4); }
			\node[fill = white] at (3,1.5) {$3$};

			\only<1-4> { \path[draw, thick] (4) -- (5); }
			\only<5-> { \path[draw, thick, blue] (4) -- (5); }
			\node[fill = white] at (4,0) {$1$};

			\only<1-5> { \path[draw, thick] (3) -- (5); }
			\only<6-> { \path[draw, thick, blue] (3) -- (5); }
			\node[fill = white] at (3,-1.5) {$2$};

			\only<1-10> { \path[draw, thick] (1) -- (3); }
			\only<1-10> { \node[fill = white] at (2,-1) {$4$}; }

			\only<1-6> { \path[draw, thick] (4) -- (6); }
			\only<7-> { \path[draw, thick, blue] (4) -- (6); }
			\node[fill = white] at (5,0.5) {$9$};

			\only<1-7> { \path[draw, thick] (6) -- (8); }
			\only<8-> { \path[draw, thick, blue] (6) -- (8); }
			\node[fill = white] at (6,-1) {$5$};

			\only<1-8> { \path[draw, thick] (8) -- (9); }
			\only<9-> { \path[draw, thick, blue] (8) -- (9); }
			\node[fill = white] at (7,-1) {$1$};

			\only<1-9> { \path[draw, thick] (7) -- (9); }
			\only<10-> { \path[draw, thick, blue] (7) -- (9); }
			\node[fill = white] at (7,1) {$5$};

			\only<1-10> { \path[draw, thick] (6) -- (9); }
			\only<1-10> { \node[fill = white] at (7,0) {$7$}; }

			\only<1-10> { \path[draw, thick] (1) -- (5); }
