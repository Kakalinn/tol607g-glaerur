...
{ // Við erum að leita að bili [x, y] og erum í [i, j].
	...
	if (x > m && y > m) return query_rec(m + 1, j, x, y, RIGHT(e));
...
void update_rec(int i, int j, int x, int y, int e) // Hjálparfall.
{ // Við erum að leita að laufinu [x, x] og erum í [i, j].
	...
	{
		int m = (i + j)/2;
		if (x <= m) update_rec(i, m, x, y, LEFT(e));
		else update_rec(m + 1, j, x, y, RIGHT(e));
		p[e] = p[LEFT(e)] + p[RIGHT(e)];
	}
}
void update(int x, int y)
{ // Bætum y við x-ta stakið.
	return update_rec(0, n - 1, x, y, 1);
}
