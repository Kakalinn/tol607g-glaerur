...
{ // Hvert er næsta stak mengisins p á eftir x?
	if (x >= p[0] - 1) return p[0];
	int i = 0, j = p[0] - 1, e = 1, s = query(p, x + 1, p[0] - 1), m;
	for (m = (i + j)/2; i != j; m = (i + j)/2)
	{
		if (x > m || s == p[RIGHT(e)]) i = m + 1, e = RIGHT(e);
		else s -= p[RIGHT(e)], j = m, e = LEFT(e);
	}
	return p[e] ? i : p[0];
}

int prev(int *p, int x)
{ // Hvað er aftasta stakið á undan x í menginu p?
	if (x <= 0) return -1;
	int i = 0, j = p[0] - 1, e = 1, s = query(p, 0, x - 1), m;
	for (m = (i + j)/2; i != j; m = (i + j)/2)
	{
		if (x <= m || s == p[LEFT(e)]) j = m, e = LEFT(e);
		else s -= p[LEFT(e)], i = m + 1, e = RIGHT(e);
	}
	return p[e] ? i : -1;
}

void insert(int *p, int x) { update(p, x, 1); }
void rem(int *p, int x) { update(p, x, 0); }
int find(int *p, int x) { query(p, x, x); }
...
{
	srand(time(NULL));
	int i, j, x, y, z, n = 10000, q = 10000000, m = 100, p[n*5], a[n];
	for (i = 0; i < n; i++) a[i] = 0;
	init(p, n);
	while (q-- != 0)
	{
		...
		a[x] = 1 - a[x];
		for (i = x - 1; x >= 0 && a[i] == 0; i--);
		assert(prev(p, x) == i);
		for (i = x + 1; x < n && a[i] == 0; i++);
		assert(next(p, x) == i);
	}
	return 0;
}
