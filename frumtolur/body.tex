...
\env{frame}
{
	\selectcode{code/miller-rabin.c}{27}{43}
}
...
\env{frame}
{
	\env{itemize}
	{
		...
		\item<3-> Reikniritið finnur þátt í samsettri tölu $n$ í $\mathcal{O}(\sqrt{a})$ tíma, þar sem $a$ er minnsti frumþáttur $n$.
		\item<4-> Nú gildir að $a \leq \sqrt{n}$ og því tekur reikniritið $\mathcal{O}(\sqrt[4]{n})$ tíma.
		\item<5-> Þetta er því töluverð bæting.
		\item<6-> Við þurfum samt fyrst úr skugga um að $n$ sé frumtala.
...
\env{frame}
{
	\selectcode{code/pollard-rho.c}{49}{64}
}

\env{frame}
{
	\selectcode{code/pollard-rho.c}{78}{96}
}
...
\env{frame}
{
	\env{itemize}
	{
		...
		\env{itemize}
		{
			...
			\[
				\sigma(n) = \prod_{k = 1}^r \frac{p_i^{e_i + 1} - 1}{p_i - 1}.
			\]
			\item<6-> Fjöldi jákvæðra heiltana $k$, þannig að $\gcd(n, k) = 1$:
			\[
				\phi(n) = n\prod_{k = 1}^r (1 - 1/p_i).
			\]
		}
