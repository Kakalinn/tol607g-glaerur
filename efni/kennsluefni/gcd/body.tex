...
\env{frame}
{
    \env{itemize}
    {
        ...
        \item<3-> Svo tímaflækjan er $\mathcal{O}($\onslide<4->{$\max(a, b)$}$)$.
        ...
        \env{frame}
{
    ...
    \only<all:8>{\slowgcdH}
    \only<all:9>{\slowgcdI}
    \only<all:10>{\slowgcdJ}
    \only<all:11>{\slowgcdK}
    \only<all:12>{\slowgcdL}
    \only<all:13>{\slowgcdM}
    \only<all:14>{\slowgcdN}
    \only<all:15>{\slowgcdO}
    \only<all:16>{\slowgcdP}
    \only<all:17>{\slowgcdQ}
    \only<all:18>{\slowgcdR}
}
...
\env{frame}
{
    \env{itemize}
    {
        ...
        \item<3-> En við getum fundið þessa tölu með \onslide<4->{leifareikningi}.
...
\env{frame}
{
    \env{itemize}
    {
        ...
        \item<5-> Ástæðan fyrir þessari bætingu er að ef \ilcode{a} minnkar lítið eftir eitt skref
            þá verður lítill munur á \ilcode{a} og \ilcode{b}, svo næst minnkar \ilcode{a} meira.
        \item<6-> Við kennum þetta reiknirit við Evklíð.
        \item<7-> Þetta ferli er einnig kallað \emph{keðjudeiling}.
    }
}

\section{Jafna Bézouts}
...
\env{frame}
{
    \env{itemize}
    {
        ...
        \item<9-> Til að koma í veg fyrir það má breyta skilagildinu í \ilcode{(x\%m + m)\%m}.
    }
}
