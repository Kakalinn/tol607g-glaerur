...
int greedy_check(int *x, int n, int k, int m)                                   // Gáum gráðugt hvort kisunum megi raða með m millibili.
{
	int i, j, r = 0;
...
int main()
{
	...
	while (r < s)                                                               // Helmingunarleit hefst.
	{
		...
		if (greedy_check(x, n, k, m)) r = m + 1;                                // Hendum burt neðri helmingnum.
