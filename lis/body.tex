\title{Lengstu vaxandi hlutrunur (\texttt{LIS})}
...
\env{frame}
{
	\env{itemize}
	{
		\item<1-> Við höfum nú þegar skoðað hvernig finna megi legstu sameiginlegu hlutrunu með tæmandi leit.
		\item<2-> Sjáum núna hvernig við getum gert það með kvikri betun.
	}
}
...
\env{frame}
{
	...
	\env{itemize}
	{
		...
		\item<5-> Við viljum nú fyrir gefna runu finna lengdina á lengu vaxandi hlutrunum rununnar.
		\item<6-> Það er oft talað um að finna ,,lengstu vaxandi hlutrununa'' þó það geti verið margar jafna langar ,,lengstu vaxandi hlutrunur''.
	}
}
...
\env{frame}
{
	\env{itemize}
	{
		...
		\item<2-> Látum $f(x)$ tákna lengd lengstu vaxandi hlutruna $(a_1, \dots, a_x)$.
		\item<3-> Þá fáum við
		\[
			f(x) = \left \{
			\env{array}
...
\env{frame}
{
	\env{itemize}
	{
		...
		\item<3-> Heildar tímaflækjan er því $\mathcal{O}($\onslide<4->{$n^2$}$)$.
...
\env{frame}
{
	\env{itemize}
	{
		\item<1-> Látum nú $f(x, y)$ tákna minnsta mögulega aftasta stak hlutrunu $(a_1, \dots, a_x)$ af lengd $y$.
		\item<2-> Við fáum því
		\[
			f(x, y) = \left \{
			\env{array}
		...
		\item<4-> Lengd lengstu vaxandi hlutrunanna er þá stærst $\ell$ þannig að $f(n, \ell)$ er ekki $\infty$. 
	}
}

\env{frame}
{
	\selectcode{code/lis2-n2-td.c}{6}{22}
}
...
\env{frame}
{
	\env{itemize}
	{
		\item<1-> Heildar tímaflækjan er sú sama og áðan, svo hún er $\mathcal{O}($\onslide<2->{$n^2$}$)$.
...
\env{frame}
{
	...
	\env{itemize}
	{
		...
		\item<5-> Í neðansæknu útfærslunni framkvæmum við línulega leit í fyrst skrefinu.
